...
export interface TargetMemoryTraceRecord { memory_id:string; canonical_value:string; scope:'profile'|'preference'|'project_requirement'|'episodic'; lifecycle_state:'ACTIVE'|'SUPERSEDED'|'CONFLICTED'; source_message_ids:string[]; observed_at?:string; write_order:number; relationships:MemoryRelationship[] }
export interface TargetMemoryTraceEvent { event_id:string; event_type:'INGESTED'|'WRITTEN'|'UPDATED'|'MAINTENANCE_APPLIED'|'CONFLICT_RECORDED'|'CONTEXTUAL_OVERRIDE_RECORDED'|'RETRIEVED'; memory_id?:string; source_message_ids:string[]; details:Record<string, unknown>; created_at:string }
...
export interface TargetMemoryTrace { run_id:string; memory_strategy:MemoryStrategy; memory_maintenance_policy:'append_only'|'update_aware_consolidation'; records:TargetMemoryTraceRecord[]; events:TargetMemoryTraceEvent[]; retrievals:TargetMemoryTraceRetrieval[] }
