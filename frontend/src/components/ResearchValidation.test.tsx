...
test('runs an authorised local benchmark baseline from the uploaded file', async () => {
  const user = userEvent.setup();
  render(<ResearchValidation />);
  await user.upload(screen.getByLabelText('LongMemEval-compatible JSON'), new File([JSON.stringify([{ id: 'B1' }])], 'benchmark.json', { type: 'application/json' }));
  ...
  await waitFor(() => expect(apiMocks.runLongMemEval).toHaveBeenCalledWith([{ id: 'B1' }], 'benchmark.json', 'strong_rule_based'));
  expect(screen.getByText('Local memory-policy baseline completed')).not.toBeNull();
