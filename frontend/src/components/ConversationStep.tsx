...
export function ConversationStep({ text, consent, busy, onTextChange, onConsentChange, onImport, onImportError, onContinue }: Props) {
  ...
  const importJson = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      onImportError('That JSON file must contain a non-empty messages array with string role and content fields.');
  ...
  return <section className="intro" aria-labelledby="new-audit-title">
    ...
    <label htmlFor="conversation-text">Authorised conversation text
      <textarea id="conversation-text" value={text} onChange={(event) => { onTextChange(event.target.value); setImportFeedback(''); }} placeholder="Example: [User] I now use PostgreSQL for the backend." rows={10} aria-describedby="conversation-help conversation-count" aria-invalid={!hasConversation} />
    </label>
