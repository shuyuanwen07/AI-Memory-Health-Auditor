...
function BenchmarkRunReport({ family, run }: { family: BenchmarkFamily; run: BenchmarkRunResponse }) {
  ...
  return <div className="research-report benchmark-run-report" role="status"><h3>{benchmarkCopy[family].label} local baseline completed</h3><p><b>{score}</b> · {run.tests_passed} / {run.tests_total} cases passed · strategy <b>{humanise(run.metadata.memory_strategy)}</b></p><div className="comparison-scroll"><table><thead><tr><th>Case</th><th>Dimension</th><th>Outcome</th><th>Retrieved records</th></tr></thead><tbody>{run.cases.map((item, index) => <tr key={item.case_id} title={`Technical case ID: ${item.case_id}`}><th scope="row">Case {index + 1}</th><td>{item.dimension ? humanise(item.dimension) : 'Not mapped'}</td><td>{item.passed ? 'Pass' : 'Fail'}</td><td>{item.retrieved_memory_ids.length ? `${item.retrieved_memory_ids.length} record${item.retrieved_memory_ids.length === 1 ? '' : 's'}` : 'None'}</td></tr>)}</tbody></table></div><details className="audit-technical-details"><summary>Technical benchmark details</summary><p>Run ID: <code>{run.metadata.run_id}</code></p><p className="fingerprint"><small>Reproducible source fingerprint</small><code>{run.metadata.source_fingerprint_sha256}</code></p></details><p><small>{run.metadata.notice}</small></p></div>;
}
...
export function ResearchValidation() {
  ...
  const [selectedStrategies, setSelectedStrategies] = useState<MemoryStrategy[]>(['weak_first_hit', 'strong_rule_based', 'scope_aware', 'temporal_importance']);
  ...
  const createMatrix = async (event: FormEvent) => {
    ...
    try {
      const labels: Record<MemoryStrategy, string> = { weak_first_hit: 'Weak first-hit', strong_rule_based: 'Strong rule-based', strong_score_based: 'Strong score-based', scope_aware: 'Scope-aware retrieval', temporal_importance: 'Temporal & importance' };
  ...
  return <section className="research-validation" aria-labelledby="research-validation-title">
    ...
    <section className="research-workspace" aria-labelledby="formal-matrix-title"><h3 id="formal-matrix-title">Formal Synthetic Pilot Matrix</h3><p>Create one frozen comparison group per synthetic scenario. Each selected strategy receives the identical human-labelled test suite; target responses are not prefilled.</p><form onSubmit={createMatrix}><div className="benchmark-controls-grid"><label>Target runtime<select aria-label="Formal matrix target runtime" value={matrixProvider} onChange={(event) => { const provider = event.target.value as TargetProvider; setMatrixProvider(provider); setMatrixModel(provider === 'ollama' ? 'qwen3:1.7b' : 'rule-based-target-ai'); }}><option value="ollama">Local Qwen via Ollama</option><option value="rule_based">Deterministic rule-based baseline</option></select><small>Use the same target model in every condition.</small></label><label>Target model<input aria-label="Formal matrix target model" value={matrixModel} onChange={(event) => setMatrixModel(event.target.value)} required /><small>Recorded in every run’s reproducibility metadata.</small></label><fieldset><legend>Memory strategies</legend>{(['weak_first_hit', 'strong_rule_based', 'strong_score_based', 'scope_aware', 'temporal_importance'] as MemoryStrategy[]).map((strategy) => <label key={strategy} className="consent"><input type="checkbox" checked={selectedStrategies.includes(strategy)} onChange={() => toggleStrategy(strategy)} /> {humanise(strategy)}</label>)}</fieldset></div><label className="consent"><input aria-label="Synthetic data confirmation" type="checkbox" checked={syntheticConfirmed} onChange={(event) => setSyntheticConfirmed(event.target.checked)} /> I confirm that this is authorised synthetic data and may be materialised as operational audit records.</label><div className="research-actions"><button type="submit" disabled={matrixBusy || !dataset || !pilotPackage || !pilotReport?.ready_for_formal_evaluation || !syntheticConfirmed || selectedStrategies.length < 2}>{matrixBusy ? 'Creating frozen matrix…' : 'Create Formal Matrix'}</button></div></form>{matrix && <div className="research-report" role="status"><h3>Formal synthetic matrix created</h3><p><b>{matrix.scenario_count}</b> scenarios × <b>{matrix.condition_count}</b> conditions = <b>{matrix.total_audit_runs}</b> controlled runs and {matrix.total_target_calls} target calls.</p><p>{matrix.notice}</p><div className="comparison-scroll"><table><thead><tr><th>Scenario</th><th>Experiment</th><th>Runs</th><th>Frozen tests</th></tr></thead><tbody>{matrix.scenarios.map((scenario, index) => <tr key={scenario.experiment_id} title={`Technical source: ${scenario.source_conversation_id}; experiment ID: ${scenario.experiment_id}`}><th scope="row">Scenario {index + 1}</th><td>Frozen comparison</td><td>{scenario.run_ids.length}</td><td>{scenario.test_count}</td></tr>)}</tbody></table></div><button type="button" disabled={matrixRunning} onClick={runMatrix}>{matrixRunning ? 'Running controlled conditions…' : `Run ${matrix.total_audit_runs} Conditions`}</button><p><small>Runs execute sequentially to respect the selected model’s local resource and request budget. Completed results appear under Experiments.</small></p></div>}</section>
