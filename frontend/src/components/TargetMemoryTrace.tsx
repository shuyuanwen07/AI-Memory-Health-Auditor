...
export function TargetMemoryTrace({ runId }: { runId: string }) {
  ...
  return <section className="target-memory-trace" aria-label="Target agent memory trace">
    ...
    {trace && <><p className="trace-note">Target system: <b>{pretty(trace.target_system_adapter ?? 'controlled-memory')}</b>{trace.target_system_adapter_version ? ` (${trace.target_system_adapter_version})` : ''} · Retrieval strategy: <b>{pretty(trace.memory_strategy)}</b> · Memory maintenance: <b>{pretty(trace.memory_maintenance_policy ?? 'update_aware_consolidation')}</b> · Capacity: <b>{trace.target_memory_capacity ?? 50}</b> records · Writer: <b>{pretty(trace.target_memory_writer ?? 'rule_based')}</b> · Values are shown only after the audit completed. Evaluator-only expected behaviour is not included here.</p>
      <h3>Stored Memory Lifecycle</h3><div className="trace-records">{trace.records.map((record) => <article className="trace-record" key={record.memory_id}><div className="row"><b>{record.memory_id}</b><span className={`trace-state ${record.lifecycle_state.toLowerCase()}`}>{pretty(record.lifecycle_state)}</span></div><p>{record.canonical_value}</p><small>Scope: {pretty(record.scope ?? 'episodic')} · Write {record.write_order} · Sources: {record.source_message_ids.join(', ') || 'none'}</small>{record.relationships.length > 0 && <ul>{record.relationships.map((relation) => <li key={relation.relationship_id ?? `${relation.type}-${relation.target_memory_id}`}>{relation.type} → {relation.target_memory_id}</li>)}</ul>}</article>)}</div>
      ...
      <h3>Retrieval Decisions</h3>{trace.retrievals.length ? <div className="trace-retrievals">{trace.retrievals.map((retrieval) => <details key={retrieval.retrieval_id}><summary><b>{retrieval.test_id}</b> · selected {retrieval.selected_memory_ids.join(', ') || 'no relevant memory'} · {retrieval.final_response_id ? `used for ${retrieval.final_response_id}` : 'superseded retry attempt'}</summary><ul>{retrieval.ranking_evidence.map((item, index) => <li key={`${String(item.memory_id)}-${index}`}>{String(item.memory_id)} — {item.selected ? 'selected' : 'not selected'}; {String(item.reason ?? 'no reason')}</li>)}</ul></details>)}</div> : <p className="empty">No retrieval decisions were recorded.</p>}</>}
  </section>;
}
