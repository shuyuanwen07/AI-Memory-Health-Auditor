...
export function TargetMemoryTrace({ runId }: { runId: string }) {
  ...
  return <section className="target-memory-trace" aria-label="Target agent memory trace">
    ...
    {trace && <><p className="trace-note">Retrieval strategy: <b>{pretty(trace.memory_strategy)}</b> · Memory maintenance: <b>{pretty(trace.memory_maintenance_policy ?? 'update_aware_consolidation')}</b> · Values are shown only after the audit completed. Evaluator-only expected behaviour is not included here.</p>
