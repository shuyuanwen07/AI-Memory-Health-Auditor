...
GitHub Actions repeats backend static compilation and tests, plus frontend type-check/build/tests, on every push and pull request. The workflow is in `.github/workflows/quality.yml`; `.editorconfig` defines the shared whitespace and indentation baseline.
...
The controlled target's private memory writer is separate from the Auditor's ground-truth extractor. Leave `TARGET_MEMORY_WRITER=rule_based` for the deterministic baseline. Set it to `llm_structured` only when the experiment's frozen pipeline provider/model is a configured cloud provider; the writer sees only authorised source messages and its version is retained in the post-audit memory trace.
...
## Local LongMemEval-compatible benchmark baseline

The repository does not include or download LongMemEval. After independently obtaining a permitted local source file, first validate its structure through `POST /api/v1/research/benchmarks/longmemeval/validate`. To exercise the controlled memory-policy baseline, call `POST /api/v1/research/benchmarks/longmemeval/run` with that same JSON payload, `source_authorised: true`, a selected `memory_strategy`, and a fixed seed. The run is ephemeral: each case receives a new in-memory store, user messages are ingested chronologically, and no source content is saved to PostgreSQL.

This runner is intentionally deterministic and provider-free. It returns a source SHA-256 fingerprint, runner/evaluator versions, case order, retrieval evidence, and category/dimension summaries. Its compact lexical reference-answer matcher is useful for repeatable integration and policy-ablation checks, but is **not** the official LongMemEval evaluator. Do not call the resulting values official LongMemEval scores; record the upstream data licence/citation and any official evaluation method separately in the research protocol.
