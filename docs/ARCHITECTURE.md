...
The research-validation router is deliberately separate from operational audit routes. It validates a frozen annotation release, calculates extraction, relationship, test-review and evaluator metrics from explicit prediction mappings, and returns a reproducible content hash. It retains no research payload. A LongMemEval-compatible adapter provides a local format boundary only; it cannot download, invoke, or claim results for the external benchmark.
