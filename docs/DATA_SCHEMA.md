...
`memory_maintenance_policy` is a run-level controlled variable. `append_only` retains both sides of an UPDATE as active observations. `update_aware_consolidation` marks the previous record `SUPERSEDED` while preserving the relationship and write evidence; it also merges only exact normalised duplicate writes (case, whitespace and punctuation insensitive), preserving all source-message IDs and an explicit event. The private event ledger records the selected policy and its maintenance action for every UPDATE or duplicate merge, so the conditions remain experimentally traceable.
